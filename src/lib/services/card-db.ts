...
async function _loadCardDatabaseImpl(): Promise<Card[]> {
	...
	// Layer 2: Bundled static JSON (always available, even offline)
	// Lazy-loaded to avoid including the ~2.7MB file in the main bundle.
	// The file may not exist in all builds (it's a dev-only artifact).
	try {
		// Dynamic import with computed path to avoid TS module resolution errors
		// when the file doesn't exist in the repo (it's generated by scripts)
		const jsonPath = '../data/card-database' + '.json';
		const module = await import(/* @vite-ignore */ jsonPath);
		const bundledCards = module.default;
		if (Array.isArray(bundledCards) && bundledCards.length > 0) {
			cards = bundledCards as Card[];
			buildIndexes();
			isLoaded = true;
			console.debug(`[card-db] Loaded ${cards.length} cards from bundled JSON`);

			// Cache in IDB for future offline use, then trigger background refresh
			try {
				await idb.setCards(cards);
				await idb.setCardsVersion('bundled-' + new Date().toISOString());
			} catch (err) {
		...
		console.debug('[card-db] Bundled JSON not available, falling through to Supabase');
	}

	// Layer 3: Full fetch from Supabase (first-time load or corrupt IDB)
