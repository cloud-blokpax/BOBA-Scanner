...
listing_canonical_unique AS (
  select
    ...
    (array_agg(canonical_card_id))[1] as canonical_card_id  -- when unique, this is the only one
...
comment on view public.canonical_listing_attributions is
  'Bug D dedup: each ebay_item_id attributed to the single best-matching card based on title discriminators across all observations. Filter on is_canonical=true.';
