...
with_ties as (
  select
    ...
    sum(case when score = top_score then 1 else 0 end)
      over (partition by ebay_item_id, observed_at) as tied_at_top
  from ranked r
)
select
  observation_id,
...
comment on view public.canonical_listing_attributions is
  'Bug D dedup: each ebay_item_id attributed to the single best-matching card based on title discriminators (card_number, parallel, weapon, name). Filter on is_canonical=true for the deduplicated set. See migration 20260512000005 for design.';
