...
**Tier 1 forensic row.** Every `runTier1` invocation persists a single `scan_tier_results` row with `tier='tier1_paddle_ocr'`, regardless of whether canonical / short-circuit / TTA produced the hit. The `extras` jsonb carries six required sub-objects: `frames` (live-OCR snapshot), `canonical` (region OCR text + per-region confidence), `detection` (method, aspect ratio, layer), `consensus` (live-vs-canonical agreement, card-number prefix validity), `catalog_lookup` (winning card, reject reason), and `decision` (path taken, miss category, would-have-hit notes). Diagnostic plumbing only — no algorithm decisions read these fields. Assembled by `src/lib/services/tier1-telemetry.ts` and persisted fire-and-forget via `writeTier1Result` in `scan-writer.ts`. Failures land in `scan_pipeline_checkpoint` with `stage='tier1_result_write:failed'`. Capped sizes: `raw_output` 16KB, `extras` 32KB, `ocr_text_raw` 2000 chars, `topn_candidates` ≤10 rows.
