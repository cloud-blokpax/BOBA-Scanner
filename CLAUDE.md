...
- **Separate docs are allowed** for canonical references: `docs/adding-a-new-game.md` (game integration checklist), `docs/game-audit.md` (multi-game architecture notes), `docs/phase-2-telemetry.md` (Phase 2 scan telemetry SQL drilldowns), and `docs/observation-schema-v1.md` (canonical row shape for archived TCG price observations — both card-scanner and TheVault conform; required reading before adding new archive sources or writing cross-app mining queries). These are maintained alongside CLAUDE.md — any change that affects them should update CLAUDE.md's reference list in the same PR.
