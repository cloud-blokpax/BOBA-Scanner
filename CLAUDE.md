...
| Wonders of The First | `wonders` | 🐉 | `cards` (game_id='wonders'), 5 rows per identity (one per parallel) | ~5,000+ |
...
│   │       │   ├── daily-maintenance/+server.ts        # Cron: Watermark-gated pruning of observations/harvest_log (7d buffer) and external_pricing_history (30d buffer); refuses if is_archive_fresh() is false
...
│   │   │   ├── tier1-telemetry.ts          # Pure function: assemble Tier 1 forensic payload from runTier1's collected state
│   │   │   ├── tier1-telemetry.types.ts    # Type contracts for the Tier 1 telemetry payload (frames/canonical/detection/consensus/catalog_lookup/decision)
...
│   ├── tier1-telemetry.test.ts          # Unit: Tier 1 PaddleOCR telemetry payload assembly + miss-category classifier
...
**Tier 1 forensic row.** Every `runTier1` invocation persists a single `scan_tier_results` row with `tier='tier1_paddle_ocr'`, regardless of whether canonical / short-circuit / TTA produced the hit. The `extras` jsonb carries six required sub-objects: `frames` (live-OCR snapshot), `canonical` (region OCR text + per-region confidence), `detection` (method, aspect ratio, layer, plus `contour_diagnostics` on the upload path — per-pass contour forensics including vertex counts at every approxPolyDP epsilon, min-area-rect aspect/angle, convex-hull area, rectangularity, and rejection-reason counters from `upload-card-detector.ts`; capped at 10 contours per pass sorted by area desc), `consensus` (live-vs-canonical agreement, card-number prefix validity), `catalog_lookup` (winning card, reject reason), and `decision` (path taken, miss category, would-have-hit notes). Diagnostic plumbing only — no algorithm decisions read these fields. Assembled by `src/lib/services/tier1-telemetry.ts` and persisted fire-and-forget via `writeTier1Result` in `scan-writer.ts`. Failures land in `scan_pipeline_checkpoint` with `stage='tier1_result_write:failed'`. Capped sizes: `raw_output` 16KB, `extras` 32KB, `ocr_text_raw` 2000 chars, `topn_candidates` ≤10 rows.

The `scan_tier` enum currently allows: `tier1_hash` and `tier1_embedding` (deprecated, retained for historical rows), `tier2_ocr` (deprecated), `tier1_paddle_ocr` (active — populated by consensus-builder convergence in `runTier1`), and `tier3_claude` (active — Haiku fallback).

Diagnostic views: `tier1_hit_rate_v1` (daily hit-rate by game / capture_source / tier value — covers both `tier1_hash` and `tier1_paddle_ocr` post-migration-074) and `tier1_diagnostic_v1` (per-scan forensic row joining `scan_tier_results` ↔ `scans` ↔ `cards`, lifting the six extras sub-objects to top-level JSON for ad-hoc queries).

Pipeline code lives in five service files: `recognition.ts` (orchestrator), `recognition-tiers.ts` (two-tier dispatcher — also the canonical convergence point for the four Tier 1 capture modes and the single owner of the `tier1_paddle_ocr` row), `recognition-validation.ts` (cross-validation), `recognition-workers.ts` (worker lifecycle), plus the Tier 1 mode implementations named above. Telemetry assembly lives in `tier1-telemetry.ts` (pure function) with type contracts in `tier1-telemetry.types.ts`.
...
**Catalog state.** Wonders cards are stored as one row per `(card_number, parallel)` tuple (matches BoBA model). Each Wonders card has up to five physical printings sharing one collector number — Paper, Classic Foil, Formless Foil, Orbital Color Match, Stonefoil — and ~99% of cards ship in all five. Documented exceptions (some tokens, set-specific exclusions) get fewer parallels via the per-set catalog's `parallels` override. `(game_id, card_number, parallel) → card_id` is the unique tuple, enforced by the `cards_game_card_parallel_unique` constraint added in migration 33. The expansion landed via migration 30; the obsolete `variant_harvest_seed` model (which inferred parallel from `A1-`/`P-` prefixes) was dropped at the same time. Card-number prefixes (`A1-`, `P-`) indicate set series (alternate art, promo) and are ORTHOGONAL to parallel — every series can ship in all five parallels.
...
**Wonders-specific fields.** Read from `cards.metadata` JSONB (orbitals, dragon points, card class, type line, rules text, set display name). The `wotf_cards` table and `wonders_cards_full` view were dropped in migration 32 — no application code referenced them, and the 1:1 `cards.id ↔ wotf_cards.id` join was broken anyway by the 5x expansion. New-set imports populate `metadata` directly via `npm run wonders:import-set`.
...
**Per-listing observation persistence.** The price-harvest cron also persists every (listing × cycle) snapshot to `ebay_listing_observations` and dedupes unique `(card_id, ebay_item_id)` rows into `ebay_card_images` via `persistObservations()` from `src/lib/server/harvester/listing-observations.ts`. Pure write-side: no HTTP fetches, no `sharp`, no Storage uploads — every column is parsed from the eBay JSON the harvester already has in memory. The filter-decision (`accepted_by_filter`, `rejection_reason`, `weapon_conflict`) is tagged onto each observation so rejected listings are queryable for filter regression hunting. Best-effort wrapper around the call — any insert failure is logged and swallowed so price harvesting (`price_cache` + `price_harvest_log`) is never blocked. Daily maintenance: `/api/cron/daily-maintenance` (triggered by `qstash-daily-maintenance`, 04:00 UTC) performs watermark-gated pruning of `ebay_listing_observations` and `price_harvest_log` (7-day buffer behind the R2 archive watermark) and `external_pricing_history` (30-day buffer), gated on `is_archive_fresh()` so a broken archive cron pauses pruning rather than risking data loss. See `docs/archive-operations.md` for the operational runbook. After each price-harvest cycle the cron also refreshes `mv_harvest_runs` (per-run summary materialized view) so the admin observability surface always reflects the latest run.
...
| Table | Purpose | Key Columns |
|-------|---------|-------------|
| `cards` | Unified card database (all games). UNIQUE `(game_id, card_number, parallel)` from migration 33. Wonders has up to 5 rows per identity (one per parallel printing). | `id` (UUID PK), `card_id_legacy` (INT UNIQUE), `name`, `hero_name`, `athlete_name`, `set_code`, `card_number`, `year`, `parallel`, `power`, `rarity`, `weapon_type`, `battle_zone`, `image_url`, `search_vector` (tsvector), `game_id` (default `'boba'`), `metadata` (JSONB default `{}`), `created_at`, `updated_at` |
...
| `scan_tier_results` | One row per tier invocation within a scan (max 2: Tier 1 + optional Tier 2). `tier` enum allows `tier1_paddle_ocr` (active — populated by `runTier1`'s finally block via `writeTier1Result`), `tier3_claude` (active — Haiku fallback DB string for "Tier 2", see naming note in Recognition Pipeline), and the legacy `tier1_hash` / `tier1_embedding` / `tier2_ocr` for pre-2.5 rows. | `id` (UUID PK), `scan_id` (FK `scans`), `user_id`, `tier` (enum `scan_tier`), `engine` (enum `scan_engine`), `engine_version`, `raw_output` (JSONB), `parsed_card_id/parallel/confidence`, `latency_ms`, `cost_usd`, `errored`, `error_message/code`, OCR fields (`ocr_text_raw`, `ocr_mean_confidence`, `ocr_word_count`, `ocr_detected_card_number`, `ocr_orientation_deg`), LLM fields (`llm_model_requested/responded`, `llm_input/output/cache_creation/cache_read_tokens`, `llm_finish_reason`, `prompt_template_sha/version`, `pricing_table_version`, `claude_returned_name_in_catalog`), hash-match legacy fields from pre-2.5 (`query_dhash`, `query_phash_256`, `match_distance`, `winner_dhash/phash_distance`, `runner_up_margin_dhash`, `hash_match_count`, `idb_cache_hit`, `sb_exact/fuzzy_hit`), `topn_candidates` (JSONB), `outcome`, `skip_reason`, `extras` (Tier 1 rows: six required sub-objects `frames` / `canonical` / `detection` / `consensus` / `catalog_lookup` / `decision`; capped at 32KB), `schema_version`, `ran_at`, `created_at` |
...
Archive & pruning (Phase 2):
- `is_archive_fresh()` — TRUE only if every `archive_watermark` row for `card-scanner` has `last_run_at` within the last 26 hours. Daily-maintenance gates pruning on this so a broken archive cron pauses pruning rather than risking data loss.
- `prune_archived_observations()` — deletes `ebay_listing_observations` rows older than `archive_watermark.last_archived_date - 7 days`. Returns `(deleted_rows, safe_cutoff, watermark_date)`.
- `prune_archived_harvest_log()` — deletes `price_harvest_log` rows older than `archive_watermark.last_archived_date - 7 days`. Returns `(deleted_rows, safe_cutoff, watermark_date)`.
- `prune_archived_external_pricing_history()` — deletes `external_pricing_history` rows older than `archive_watermark.last_archived_date - 30 days` (longer buffer; sales-history strategic asset kept hot for ad-hoc queries). Returns `(deleted_rows, safe_cutoff, watermark_date)`.
...
Wonders cards have different attributes than BoBA: `type_line`, `subtype`, `card_class`, `hierarchy`, `lineage`, `faction`, `orbital_cost`, `orbitals`, `cost`, `power`, `rules_text`, `traits`. These live in the `metadata` JSONB column on the unified `cards` table. (The legacy `wotf_cards` table and `wonders_cards_full` view were dropped in migration 32 — `cards.metadata` is the sole source.)
...
- **Separate docs are allowed** for canonical references: `docs/adding-a-new-game.md` (game integration checklist), `docs/game-audit.md` (multi-game architecture notes), `docs/phase-2-telemetry.md` (Phase 2 scan telemetry SQL drilldowns), `docs/observation-schema-v1.md` (canonical row shape for archived TCG price observations — both card-scanner and TheVault conform; required reading before adding new archive sources or writing cross-app mining queries), and `docs/archive-operations.md` (operational reference for the R2 archive + watermark-gated pruning system; required reading before manually triggering archive or pruning). These are maintained alongside CLAUDE.md — any change that affects them should update CLAUDE.md's reference list in the same PR.
...
- **NEVER** move BoBA columns (`hero_name`, `weapon_type`, `athlete_name`, `battle_zone`) into `metadata` JSONB. These are first-class columns by design. Wonders-specific fields (`cost`, `hierarchy`, `card_class`, `orbitals`, etc.) live in `cards.metadata`. The previous `wotf_cards` table was dropped in migration 32 — don't reintroduce a separate Wonders-only reference table.
